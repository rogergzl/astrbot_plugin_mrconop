...
function adEm(idx){var em=window._emacros||[],v=idx!==undefined?em[idx]:null;var etOpts='<option value="player_death" '+(v&&v.event_type==="player_death"?"selected":"")+'>玩家死亡</option><option value="player_join" '+(v&&v.event_type==="player_join"?"selected":"")+'>玩家加入</option><option value="player_leave" '+(v&&v.event_type==="player_leave"?"selected":"")+'>玩家离开</option><option value="player_advancement" '+(v&&v.event_type==="player_advancement"?"selected":"")+'>玩家成就</option>';var h='<div class="mbg" id="em-m" onclick="if(event.target===this)this.remove()"><div class="mod" style="max-width:560px"><h3>'+(v?"编辑":"新建")+'事件宏</h3><div class="fg"><label>名称 *</label><input id="emn" value="'+(v?v.name:"")+'"></div><div class="fg"><label>事件类型</label><select id="emet">'+etOpts+'</select></div><div class="fg"><label>目标服务器 *</label><input id="emsn" placeholder="与服务器管理中名称一致" value="'+(v?v.server_name||"":"")+'"></div><div class="fg"><label>RCON命令（一行一条）</label><textarea id="emcmds" rows="4" placeholder="say {player} 死了！&#10;give {player} apple 1">'+(v?(v.commands||[]).join("\n"):"")+'</textarea></div><div class="fg"><label class="tg"><input type="checkbox" id="emen" '+(v&&v.enabled?"checked":"")+'><span class="sl"></span> 启用</label></div><div class="fg"><label>冷却时间（秒）</label><input id="emcd" type="number" value="'+(v?v.cooldown||0:0)+'" min="0" step="1"></div><div style="margin-top:14px"><button class="b1 bs" onclick="doEmSave('+idx+')">保存</button><button class="b2 bs" onclick="document.getElementById(\'em-m\').remove()">取消</button></div></div></div>';document.body.insertAdjacentHTML("beforeend",h)}
function edEm(idx){adEm(idx)}
async function delEm(idx){if(!confirm("确定要删除此事件宏吗？"))return;await fetch(A+"/event-macros/"+idx,{method:"DELETE"});toast("已删除");loSc()}
async function tglEm(idx,enabled){await pjt(A+"/event-macros/"+idx,{enabled:enabled});toast(enabled?"已启用":"已禁用");loSc()}
async function doEmSave(idx){var b={name:document.getElementById("emn").value.trim(),event_type:document.getElementById("emet").value,server_name:document.getElementById("emsn").value.trim(),commands:document.getElementById("emcmds").value.split("\n").map(function(s){return s.trim()}).filter(Boolean),enabled:document.getElementById("emen").checked,cooldown:parseFloat(document.getElementById("emcd").value)||0};if(!b.name||!b.server_name){toast("请填写名称和目标服务器",true);return}if(idx!==undefined&&idx>=0){await pjt(A+"/event-macros/"+idx,b)}else{await pj(A+"/event-macros",b)}document.getElementById("em-m")?.remove();toast("保存成功");loSc()}
/* ====== 日志查看器 ====== */
var _lv_type="",_lv_server="",_lv_player="";
async function loLogViewer(){try{var srvs=await fj(A+"/log-viewer/servers");var lv=await doLvFetch();var cfg={};try{cfg=await fj(A+"/config/all")}catch(e){}var h='';h+='<div class="fb" style="margin-bottom:12px"><h3 style="margin:0">📄 日志查看器</h3><button class="b1 bsm" onclick="refLog()" style="margin-left:auto" title="手动刷新">🔄 刷新</button></div>';h+='<div class="ct" style="padding:14px"><div class="fr" style="gap:16px;align-items:center;flex-wrap:wrap;margin-bottom:14px"><span style="font-size:13px;font-weight:600;white-space:nowrap">📡 监听</span><select id="lv-listen" onchange="tglLogListen(this.value)" style="padding:4px 8px"><option value="0" '+(!cfg.log_listener_enabled?'selected':'')+'>关闭</option><option value="1" '+(cfg.log_listener_enabled?'selected':'')+'>开启</option></select><select id="lv-srv" onchange="lvSrvChange(this.value)" style="padding:4px 8px"><option value="">全部服务器</option>';if(srvs&&srvs.length)srvs.forEach(function(s){h+='<option value="'+s.name+'" '+(_lv_server===s.name?'selected':'')+'>'+s.name+'</option>'});h+='</select><div style="display:flex;align-items:center;gap:8px;margin-left:auto"><span style="font-size:11px;color:var(--a);white-space:nowrap">⏱ 间隔</span><input id="lv-refresh-ms" type="number" value="'+(cfg.log_viewer_refresh_ms||10000)+'" min="0" step="1000" style="width:72px;text-align:center;padding:3px 4px" title="0=关闭自动刷新"><span style="font-size:10px;color:var(--m)">ms</span><button class="b2 bsm" onclick="tglLogRefresh()">应用</button></div></div>';var curSrv=null,curMode='file',curPaths=[],curFolder='',curFPattern='latest.log';if(_lv_server){for(var i=0;i<srvs.length;i++){if(srvs[i].name===_lv_server){curSrv=srvs[i];curMode=srvs[i].log_mode||'file';curPaths=srvs[i].log_paths||[];curFolder=srvs[i].log_folder||'';curFPattern=srvs[i].log_file_pattern||'latest.log';break}}}h+='<div style="padding-top:12px;border-top:1px solid var(--b)"><div class="fr" style="gap:14px;align-items:center;margin-bottom:8px;flex-wrap:wrap"><span style="font-size:13px;font-weight:600;white-space:nowrap">📁 日志路径</span><span style="font-size:10px;color:#e74c3c;margin-left:8px;white-space:nowrap">⚠ Docker中MC在另一容器时需先配置卷挂载才能读取日志文件</span><label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:12px;white-space:nowrap"><input type="radio" name="lv-mode" value="file" '+(curMode==='file'?'checked':'')+' onchange="lvModeChange(this.value)" id="lv-mode-file"> 文件模式</label><label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:12px;white-space:nowrap"><input type="radio" name="lv-mode" value="folder" '+(curMode==='folder'?'checked':'')+' onchange="lvModeChange(this.value)" id="lv-mode-folder"> 文件夹模式</label><button class="b1 bsm" onclick="saveLogPath()" style="margin-left:4px">💾 保存</button><button class="bw bsm" onclick="testLogFile()" style="margin-left:4px" title="读取日志最后100行检测可读性">🔍 检测</button></div><div id="lv-file-section" style="display:'+(curMode==='folder'?'none':'block')+'"><textarea id="lv-logpaths" placeholder="一行一个日志文件路径&#10;如 D:/mc/logs/latest.log" rows="3" style="width:100%;max-width:550px;font-size:11px">'+escapeHtml(curPaths.join('\n'))+'</textarea></div><div id="lv-folder-section" style="display:'+(curMode==='folder'?'block':'none')+'"><div class="fr" style="gap:8px"><input id="lv-logfolder" placeholder="文件夹路径，如 D:/mc/logs" value="'+escapeHtml(curFolder)+'" style="flex:1;max-width:390px;font-size:11px"><input id="lv-logfpattern" placeholder="文件名，如 latest.log" value="'+escapeHtml(curFPattern)+'" style="width:160px;font-size:11px"></div></div></div></div>';h+='<div class="fb" style="margin-top:10px;gap:8px;flex-wrap:wrap;align-items:center"><div id="lv-tabs" style="display:flex;gap:4px;flex-wrap:wrap">';var tabs=[["","全部"],["chat","💬聊天"],["command","⌨️命令"],["player_death","💀死亡"],["player_join,player_leave","🚪进出"],["player_advancement","⭐成就"],["system","⚙️系统"]];tabs.forEach(function(t){h+='<button class="b'+(_lv_type===t[0]?'1':'2')+' bsm" onclick="_lv_type=\''+t[0]+'\';loLogViewer()">'+t[1]+'</button>'});h+='</div><input id="lv-player" value="'+_lv_player+'" placeholder="👤 玩家筛选" onchange="_lv_player=this.value;loLogViewer()" style="width:120px;margin-left:auto"></div>';h+='<div class="ct" style="margin-top:8px" id="lv-list">'+buildLvList(lv)+'</div>';document.getElementById("main").innerHTML=h;document.getElementById("lv-srv").value=_lv_server;refresh(cfg.log_viewer_refresh_ms||10000,async function(){var l=await doLvFetch();var el=document.getElementById("lv-list");if(el)el.innerHTML=buildLvList(l)})}catch(e){document.getElementById("main").innerHTML='<div class="ct"><h3>📄 日志查看器</h3><div class="emp">加载失败: '+e.message+'<br><small>请确认已配置服务器并开启日志监听</small></div></div>'}}function lvSrvChange(v){_lv_server=v;loLogViewer()}function lvModeChange(v){document.getElementById("lv-file-section").style.display=v==="file"?"block":"none";document.getElementById("lv-folder-section").style.display=v==="folder"?"block":"none"}async function tglLogListen(v){try{await pjt(A+"/config/save",{log_listener_enabled:v=="1"});toast("日志监听已"+(v=="1"?"开启":"关闭")+"，需重启插件生效")}catch(e){toast("操作失败: "+e.message,true)}}async function saveLogPath(){if(!_lv_server){toast("请先选择服务器",true);return}var mode=document.getElementById("lv-mode-file").checked?"file":"folder";var data={server_name:_lv_server,log_mode:mode};if(mode==="file"){var txt=document.getElementById("lv-logpaths").value;data.log_paths=txt.split("\n").map(function(s){return s.trim()}).filter(Boolean)}else{data.log_folder=document.getElementById("lv-logfolder").value.trim();data.log_file_pattern=document.getElementById("lv-logfpattern").value.trim()||"latest.log"}try{var r=await pjt(A+"/log-viewer/log-path",data);toast("已保存，更新了 "+r.updated+" 个服务器配置")}catch(e){toast("保存失败: "+e.message,true)}}async function tglLogRefresh(){var v=parseInt(document.getElementById("lv-refresh-ms").value)||10000;try{await pjt(A+"/config/save",{log_viewer_refresh_ms:v});toast("已设为 "+v+"ms，重新进入生效")}catch(e){toast("保存失败: "+e.message,true)}}
...
async function doSaveCfg(){
    var b={
        ...
        relay_mc_to_group_log:document.getElementById("cfg-relay-m2gl").value=="1",
