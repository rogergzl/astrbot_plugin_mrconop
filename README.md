...
![Version](https://img.shields.io/badge/version-v3.11.1-green)

RCON 命令转发 + 服务器并发查询 + SQLite 玩家数据库 + 会话级在线追踪 + 签到 + 补偿 + 群服消息互联（RCON tellraw）

## 安装

本目录作为 AstrBot 插件加载。依赖 `mcstatus`（首次运行自动安装，无需手动 pip install）。
...
| `/mcget <名称>` | `/服详情` | 全员 | 查询单个服务器详情 |
| `/mclist` | `/服列表` | 全员 | 列出本群所有服务器 |
| `/在线时长 [服名\|玩家名]` | `/onlinetime` | 全员 | 在线时长排行（默认本群） |
| `/在线提醒` | — | 管理员 | 查看/配置本群在线提醒与踢出 |
| `/mcset <项名> <0\|1>` | `/查询设置` | 管理员 | 设置查询显示项 |

### MC 服务器管理

| 命令 | 别名 | 权限 | 说明 |
|------|------|------|------|
| `/mcadd <名称> <地址>` | `/加服` | 管理员 | 添加 MC 服务器 |
| `/mcdel <名称>` | `/删服` | 管理员 | 删除服务器 |
| `/mcup <名称> [新名] [新址]` | `/改服` | 管理员 | 更新服务器 |
| `/mcshare <名称> <目标群ID>` | `/共享服` | 超管 | 共享服务器到其他群 |
| `/mcunshare <名称> <目标群ID>` | `/取消共享` | 超管 | 取消共享 |
...
| `/宏 <名称> [参数]` | `/rcmacro` | 白名单 | 执行预定义宏 |
| `/脚本 <文件名>` | `/rcscript` | 白名单 | 执行脚本文件 |
| `/rc赞同` `/rc反对` | — | 全员 | 投票 |
| `/rc通过` `/rc否决` | — | 管理员 | 裁决 |
...
| 命令 | 别名 | 权限 | 说明 |
|------|------|------|------|
| `/msay <消息>` | `/群说` `/服说` | 全员 | 主动发送消息到 MC 公屏 |
| `/消息互通` | `/relay` | 管理员 | 查看/配置本群消息互通。子命令: 开/关/模式/群到服/服到群/格式/服/重置 |
| `/消息互通 模式 <off\|global\|custom>` | — | 管理员 | 设置模式（不互通/遵循全局/独立配置） |
| `/消息互通 群到服 <开\|关>` | — | 管理员 | 独立配置群→服转发 |
| `/消息互通 服到群 <开\|关>` | — | 管理员 | 独立配置服→群转发 |
| `/消息互通 仅msay <开\|关>` | — | 管理员 | 仅允许 /msay 命令互通（不自动转发群消息） |
| `/rchelp` | `/帮助` `/help` | 全员 | 查看所有命令帮助 |

### Web 管理面板

启用后可通过浏览器图形化管理所有配置（基于原生 `asyncio` TCP，零外部依赖）：

| 页面 | 端口 | 权限 | 说明 |
|------|------|------|------|
| Web 面板 | `9949`（可配置） | 本地访问 | 15 个功能模块的图形化管理 |
...
| 🖥️ 服务器管理 | 图形化增删改查 RCON 槽位（含所有参数），编辑弹窗带字段悬浮提示 |
| 🔗 群服互联 | 全局开关+格式+仅msay配置 / 群模式选择（不互通/遵循全局/独立） / 群→服 & 服→群 & 仅msay独立开关 |
| 📡 在线追踪 | 全局可编辑 + 按群覆盖配置，支持排行定时自动重置 |
| 👥 玩家管理 | 手动增删改玩家数据、导入在线玩家、支持修改QQ号和MC ID |
| 📋 补偿审批 | 待处理/已通过/已拒绝分类管理 |
| 🟢 在线列表 | 卡片式展示 + 在线时段横向柱状图（可滚动） + 在线时长排行（支持手动重置） |
| ⚡ 快捷命令 | 服务器预设命令（白天/黑夜/游戏规则） + 每玩家独立快捷命令（give/nick/FTB） |
| 📜 审计日志 | 分类筛选（命令/Web操作/Web命令）+ 自动刷新，显示群聊名称和操作类别 |
| ⚙️ 全局设置 | 双列卡片布局，配置文件中所有参数均可图形化修改 |
| 🔧 高级功能 | 白名单/公开命令/投票/共享/互联等每服独立配置 |
...
> 例: `§c[§l提醒§r§c] §e{player} §a已连续在线 §b{duration}`
...
> **主动转发**：使用 `/msay <消息>`（别名 `/群说` `/服说`）可主动将消息发送到 MC 公屏，无需等待自动转发。
> 
> **群内覆盖**：管理员可通过 `/消息互通` 命令按群覆盖以上配置（开关/格式/目标服务器），无覆盖时回退全局默认。
> 
> **独立服务器分配**：`/消息互通 服 <名称>` 可为不同群指定不同的 MC 服务器作为消息转发目标。
> 
> **多服务器绑定**：Web 面板支持一配置多服（群→服A + 群→服B），可手动添加额外绑定条目。
>
> **子命令**: `开|关|模式|群到服|服到群|仅msay|格式|服|重置`，直接 `/消息互通` 查看当前状态。
>
> **⚠ 服→群转发需 MC 配套模组**：服→群方向当前暂不可用，需在 MC 服务端安装配套模组。模组发布后可从 [GitHub Releases](https://github.com/rogergzl/astrbot_plugin_mrconop) 下载。群→服（QQ→MC）已可用，无需额外安装。Web 面板群服互联页有下载跳转链接，开发中。
...
> 
> 面板功能：仪表盘、服务器管理（含投票配置）、群服互联（自动绑定+多服绑定）、在线追踪、玩家管理、补偿审批、在线列表+时段图表+排行、快捷命令（预设+FTB）、审计日志、全局设置（双列布局）。
