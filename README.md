...
![Version](https://img.shields.io/badge/version-v3.9.6-green)
...
| `/消息互通` | `/relay` | 管理员 | 查看/配置本群消息互通状态与转发 |
...
| 🔗 群服互联 | 自动绑定 / 手动多服务器绑定 / 群→服 & 服→群独立开关 / 全局互联交互配置 |
| 📡 在线追踪 | 按群覆盖配置追踪参数的图形界面 |
| 👥 玩家管理 | 手动增删改玩家数据、导入在线玩家 |
| 📋 补偿审批 | 待处理/已通过/已拒绝分类管理 |
| 🟢 在线列表 | 卡片式展示 + 在线时段横向柱状图（可滚动） + 在线时长排行 |
| ⚡ 快捷命令 | 服务器预设命令（白天/黑夜/游戏规则） + 每玩家独立快捷命令（give/nick/FTB） |
| 📜 审计日志 | RCON 命令审计，自动刷新，显示群聊名称 |
...
>
> **子命令**: `开|关|格式|服|重置`，直接 `/消息互通` 查看当前状态。
