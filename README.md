...
![Version](https://img.shields.io/badge/version-v3.20.0-green)

RCON 命令转发 + 服务器并发查询 + SQLite 玩家数据库 + 在线追踪 + 签到 + 补偿 + 积分兑换 + 抽奖 + 群服消息互联（RCON tellraw）
...
> ```
> `Pillow` 可选，安装后 `/rchelp` 命令将以深色主题图片渲染，未安装时回退纯文本。

## 架构

```
mrcon/
├── main.py           # 插件入口（命令路由 + 业务逻辑 + 图片渲染）
...
| `MC→群(日志) ~0.5s 延迟` | — | 管理员(Web) | 变相服→群互通，无需 MC 模组 |

> 在全局设置中开启 "RCON日志监听" + "MC→群(日志)"，并在服务器管理中填写日志路径，即可通过日志文件变相实现服→群消息转发，延迟约 0.5~1 秒。支持聊天/进出/死亡/成就事件。

### 帮助

| 命令 | 别名 | 权限 | 说明 |
|------|------|------|------|
| `/rchelp` | `/帮助` `/help` | 全员 | 深色主题图片帮助（安装 Pillow 后） |

> 安装 `Pillow` 后 `/rchelp` 渲染为深色主题 PNG 图片（Catppuccin Mocha 配色），未安装时回退纯文本摘要。
...
| 💬 群服互联 | 全局开关+格式+仅msay配置 / 群模式选择（不互通/全局/独立） / 群→服 & 服→群独立开关 / 多服绑定 |
...
>
> **⚠ 服→群转发需 MC 配套模组**：服→群方向当前暂不可用，需在 MC 服务端安装配套模组。模组发布后可从 [GitHub Releases](https://github.com/rogergzl/astrbot_plugin_mrconop) 下载。
