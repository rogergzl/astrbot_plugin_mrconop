...
class WebServer:
    ...
    async def _api_tracker_set(self, gid: str, body: bytes) -> bytes:
        ...
        mapping = {
            ...
            "ban_minutes": True, "duration_mode": True,
    ...
    def _api_online(self, params: dict = None) -> bytes:
        ...
        pending_unbans: dict = getattr(self.plugin, '_pending_unbans', {}) or {}
        now = int(time.time())
        filter_gid = None
        if params:
        ...
        banned_players = {}
        for ban_key, ub in pending_unbans.items():
            ...
            gid = str(conf.get("group_id", ""))
            if filter_gid and gid != filter_gid:
                continue
            if gid and not self._is_group_web_managed(gid):
                continue
            remaining = max(0, ub.get("unban_at", 0) - now)
            banned_players.setdefault(srv_name, {})
            banned_players[srv_name][pname] = {
                "unban_at": ub.get("unban_at", 0),
